    ...
    save() {
        ...
        if (this.categorie.id !== undefined) {
            this.subscribeToSaveResponse(
                this.categorieService.update(this.categorie), false);
        } else {
            this.subscribeToSaveResponse(
                this.categorieService.create(this.categorie), true);
        }
    }

    private subscribeToSaveResponse(result: Observable<Categorie>, isCreated: boolean) {
        result.subscribe((res: Categorie) =>
            this.onSaveSuccess(res, isCreated), (res: Response) => this.onSaveError(res));
    }

    private onSaveSuccess(result: Categorie, isCreated: boolean) {
        this.alertService.success(
            isCreated ? 'g2SitApp.categorie.created'
            : 'g2SitApp.categorie.updated',
